...
function ensureSeedData(db: DatabaseSync) {
  ...
  db.prepare("INSERT INTO categories (id, name, slug) VALUES (?, ?, ?)").run(
  ...
  db.prepare(
    `
      INSERT INTO suppliers (id, name, contact_name, email, phone, is_active, created_at)
  ...
  db.prepare(
    `
      INSERT INTO suppliers (id, name, contact_name, email, is_active, created_at)
      VALUES (?, ?, ?, ?, 1, CURRENT_TIMESTAMP)
    `
  ).run(
  ...
  insertCount.run(randomUUID(), 18, new Date(Date.now() - 90 * 60 * 1000).toISOString(), products[0].id, employeeId);
  insertCount.run(randomUUID(), 6, new Date(Date.now() - 50 * 60 * 1000).toISOString(), products[2].id, employeeId);
  insertCount.run(randomUUID(), 9, new Date(Date.now() - 20 * 60 * 1000).toISOString(), products[4].id, adminId);
}
